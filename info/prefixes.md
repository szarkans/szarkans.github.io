---
aside: false
---

# Префиксы и суффиксы в игре

::: tip Обозначения
**Префикс** - иконка **перед** ником. **Суффикс** - после ника.
:::

## Префиксы

| Иконка | Что обозначает? |
| :------- | :--------------- | 
|![Префикс Админа](/assets/general/prefixes/admin.webp){ width=20 }|Префикс Всекотов - администрации.|
|![Префикс Модератора](/assets/general/prefixes/moder.webp){ width=20 }|Префикс Всекотят - модерации.|
|![Префикс К+++](/assets/general/prefixes/kplusplusplus.webp){ width=20 }|Префикс <span class="neon">Котика+</span>. Подробнее [на этой странице](./donate.md).|
|![Кадет КСБ](/assets/general/prefixes/kadet_police.webp){ width=20 }|Префикс **кадета** КСБ.|
|![Мл. Офицер КСБ](/assets/general/prefixes/police_junior.webp){ width=20 }|Префикс **Младшего Офицера** КСБ.|
|![Офицер КСБ](/assets/general/prefixes/police.webp){ width=20 }|Префикс **Офицера** КСБ.|
|![Ст. Офицер КСБ](/assets/general/prefixes/police_senior.webp){ width=20 }|Префикс **Старшего Офицера** КСБ.|
|![Глава КСБ](/assets/general/prefixes/glav_police.webp){ width=20 }|Префикс **Главы** КСБ.|
|![Репортёр](/assets/general/prefixes/reporter.webp){ width=20 }|Префикс Репортёров.|
|![Гид](/assets/general/prefixes/guide.webp){ width=20 }|Префикс Гидов.|
|![Ютубер](/assets/general/prefixes/youtube.webp){ width=20 }|Префикс Ютубера. Можно получить, став [нашим партнёром](./for_media.md).|
|![Стример](/assets/general/prefixes/twitch.webp){ width=20 }|Префикс Стримера. Можно получить, став [нашим партнёром](./for_media.md).|
|![Тиктокер](/assets/general/prefixes/tiktok.webp){ width=20 }|Префикс Тиктокера. Можно получить, став [нашим партнёром](./for_media.md).|
|![В тюрьме](/assets/general/prefixes/jailed.webp){ width=20 }|Префикс заключённого. Даётся автоматически, если игрок в тюрьме.|
|![АФК](/assets/general/prefixes/afk.webp){ width=35 }|Префикс АФК. Даётся автоматически, если игрок неактивен.|
|<span class="red">●</span>|У котика нет войсчата|
|<span class="red">✕</span>|У котика не установлен ресурспак|

## Суффиксы

| Иконка | Что обозначает? |
| :------- | :--------------- | 
|![newbie](/assets/general/prefixes/newbie.webp){ width=20 }|Суффикс **новичка** - даётся автоматически, пока игрок не наиграет 10 часов.|
|![wanted](/assets/general/prefixes/wanted.webp){ width=20 }|Суффикс розыска - даётся автоматически при объявлении игрока в розыск.|